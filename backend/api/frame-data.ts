 ...
 */
router.get('/video/:videoId/frame/:frameIndex', async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/video/:videoId/preload', async (req: Request, res: Response) => {
  try {
 ...
 */
router.delete('/video/:videoId/cache', async (req: Request, res: Response) => {
  try {
    const { videoId } = req.params;
    ...
    if (frameIndex) {
      const frameIdx = parseInt(frameIndex as string, 10);
      if (isNaN(frameIdx)) {
        return res.status(400).json({ error: 'Invalid frameIndex' });
      }
