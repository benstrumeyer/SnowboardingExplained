...
import { storeReferenceStackedPosition, getReferenceStackedPosition, getDefaultStackedPosition, getStackedPositionsForStance, updateReferenceStackedPosition, setAsDefaultStackedPosition, deleteReferenceStackedPosition, } from '../src/services/referenceStackedPositionService';
import { compareStackedPositions, generateStackedPositionFeedback, calculateAverageStackedPosition, } from '../src/utils/stackedPositionAnalyzer';
 ...
 * }
 */
router.post('/stacked-position/reference', async (req, res) => {
    try {
        ...
        // Store the reference
        const reference = await storeReferenceStackedPosition(name, description, stance, frame, trick, acceptableRanges);
        res.json({
            success: true,
            data: reference,
        });
 ...
 */
router.get('/stacked-position/reference/:id', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/stacked-position/default/:stance', async (req, res) => {
    try {
        ...
        const reference = await getDefaultStackedPosition(stance);
        if (!reference) {
            return res.status(404).json({
 ...
 */
router.get('/stacked-position/list/:stance', async (req, res) => {
    try {
        const { stance } = req.params;
        if (!['regular', 'goofy'].includes(stance)) {
            return res.status(400).json({
        ...
        const references = await getStackedPositionsForStance(stance);
        res.json({
            success: true,
            data: references,
        });
 ...
 * }
 */
router.post('/stacked-position/analyze', async (req, res) => {
    try {
 ...
 */
router.put('/stacked-position/reference/:id', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post('/stacked-position/reference/:id/set-default', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.delete('/stacked-position/reference/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
//# sourceMappingURL=stacked-position.js.map