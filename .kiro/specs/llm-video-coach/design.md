...
**Phase Structure:**
1. **Setup Phase**: Rider approaches the feature, initiates edge control, prepares for rotation
2. **Windup Phase**: Rider loads the board, initiates upper/lower body separation, prepares for snap
3. **Throw Phase**: Rider executes the snap, rotates the board, commits to the trick
4. **Lip Timing Phase**: Rider times takeoff with the feature, maintains rotation momentum
5. **Takeoff Phase**: Rider leaves the feature, completes rotation, prepares for landing
...
interface Video {
  ...
  phaseBoundaries?: {
    ...
    windup: { startFrame: number; endFrame: number };
    throw: { startFrame: number; endFrame: number };
    lipTiming: { startFrame: number; endFrame: number };
...
interface PhaseBasedCoachingFeedback {
  trickName: string;
  phases: Array<{
    phaseName: "setup" | "windup" | "throw" | "lipTiming" | "takeoff";
...
*For any* coaching feedback generated by the LLM, all identified issues SHALL be associated with the correct phase, reference specific frame numbers within that phase, and be grounded in phase-specific requirements.
...
*For any* motion inference across frames within a phase, the LLM's description of movement SHALL be consistent with the pose keypoint trajectories across those frames and aligned with phase-specific requirements.
